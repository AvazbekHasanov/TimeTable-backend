import pool from '../db.js'; // Assuming db connection is handled here
...
export const getStudentTimetable = async (startDate, studentId, teacherId) => {
    const query = `
        SELECT
            c.id,
            c.week_of_day,
   ...
   WHERE
        ...
        (case when $2::INTEGER is not null then (CASE
        WHEN $2::INTEGER IS NOT NULL THEN dt.teacher_id = $2::INTEGER
        ELSE true
        END) else c.science_group_id IN (
        SELECT science_group_id FROM science_group_students WHERE student_id = coalesce($3, 1)
        ) end)
        AND c.state = 1
        ORDER BY c.week_of_day;
    `;
    return await pool.query(query, [startDate, teacherId, studentId]);
};
